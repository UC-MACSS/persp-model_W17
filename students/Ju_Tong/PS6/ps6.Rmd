...
Interpretation of co-effecients in terms of log odds, odds:
As each one year increase in age, we expect the log-odds of voting status to increase by 0.04388;and expect to see see 104.49% increase in the odds of voting, when holding other variables constant.
As each one year increase in education year, we expect the log-odds of voting status to increase by 0.2384; and expect to see see 126.93% increase in the odds of voting, when holding other variables constant.
As each 10k increase in annual income, we expect the log-odds of voting status to increase by 0.08077; and expect to see see 108.41% increase in the odds of voting, when holding other variables constant.
Married people has higher log odds of voting than unmarried ones by factor of 0.3279; or say, Married people has increased their odds of voting by 138.81% than those unmarried ones, when controlling all the other variables. 
...
tv_poisson<- gss %>%
  ...
  add_predictions(poisson_1, var = 'logit') %>%
  mutate(prob = logit2prob(logit)) %>%
  mutate(odds = prob2odds(prob))
