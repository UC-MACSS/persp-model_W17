...
```
Thus, we see that `educ`, i.e. years of education, is the most impactful non-binary predictor on predicted voting probability, and `married` also results in a shift upward in predicted probability.
...
```
From the above table, we see the backwards AIC selection has resulted in a model with 4 predictor variables, not including the intercept. All of the predictors are significant at the `.05` level, except `grass`, which is only significant at the `.1` level.  With a .5 threshold, the accuracy rate of this model is `r round(100 * bsel_crit$acc_rate, 2)`%, the proportional reduction in error over the useless classifier is `r round(100 * bsel_crit$pre, 2)`%, and the AUC (over all threshold values) is `r bsel_crit$auc`.
