...
set.seed(1234)
fitted4 <- predict(boosting_depth1, newdata = mh_split$test, n.trees=10000)
...
boosting_depth3 = gbm(vote96 ~ ., 
                      data = mh_split$train, 
                      n.trees = 10000, 
                      interaction.depth = 3)
```

```{r problem_2_5_fit, echo=FALSE}
fitted5 <- predict(boosting_depth3, newdata = mh_split$test, n.trees = 10000, decision.values = TRUE) %>%
  attributes

roc_depth3 <- roc(as.numeric(as_tibble(mh_split$test)$vote96), as.numeric(fitted4))
plot(roc_depth3)
```

```{r problem2_5_fit, echo=FALSE}
set.seed(1234)
fitted5 <- predict(boosting_depth1, newdata = mh_split$test, n.trees=10000)
