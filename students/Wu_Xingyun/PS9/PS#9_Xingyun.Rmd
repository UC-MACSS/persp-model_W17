...
```
  ...
  It turns out that `Private is not strongly correlated with other variables. To make my PCA result more precise, I have to ignore it and run PCA with other varialbs in the following analysis. The first two principal components are shown in the plot below.
  
```{r problem3_model, echo=FALSE}
pr.out <- prcomp(data3[, 2:18], scale = TRUE)
biplot(pr.out, scale = 0, cex = .8, xlabs=rep(".", nrow(data3)))
pr.out$rotation
```
...
### 8. Hierarchically cluster with complete linkage and Euclidean distance
