...
The quantiles of the repeated 100 CV MSE values are 340.8504077, 389.7264343, 400.1067017, 414.8313229, 455.3244605, and mean is 401.6489239，and standard deviation is 20.7385133. We can see the mean and median is very close to the MSE value in step 1 and 2.
...
The quantiles of the repeated 100 10-fold CV MSE is 396.7948745, 397.7133823, 398.0782456, 398.4878046, 399.4557604, the mean is 398.0997361, and the standard deviation is 0.5076214. We can see that the 10-fold CV MSEs are very close.
...
The cubic tranformed model is much better than the original one according to the MSE value.

Therefore, we can conclude that the relationship between out-of-state tuition and the percent of faculty with Ph.D.'s is statistically significant (p-value: 2.759952e-31, 1.042510e-17, 8.872413e-04); when the percent of faculty with Ph.D.'s is lower than around 70%, the relationship is substantively insignificant, while when the precentage is higher than 70%, the relationship is positive and substantively significant.
...
It seems not fit the data well. Let's try the regression splines. We can estimate 10-fold CV MSE for varying numbers of polynomial degrees and knots.
...
The spline model is better than the original one according to the MSE value.
...
It seems that the MSE is lowest at the 1 polynomial degrees, which means the linear model best fits the data.
...
![](PS7_files/figure-markdown_github/unnamed-chunk-30-1.png)![](PS7_files/figure-markdown_github/unnamed-chunk-30-2.png)![](PS7_files/figure-markdown_github/unnamed-chunk-30-3.png)![](PS7_files/figure-markdown_github/unnamed-chunk-30-4.png)![](PS7_files/figure-markdown_github/unnamed-chunk-30-5.png)![](PS7_files/figure-markdown_github/unnamed-chunk-30-6.png)

As shown above, the 6 predictors in this model are all statistically significant. Being a private university the out-of-state tuition will increase by 2511.4994934 dollars. With room-board costs increasing by 1 dollar, the tuition will increase by 0.9766610 dollars. With faculty with Ph.D.'s increasing 1 percent, the tuition will increase by 26.5725050 dollars. With alumni who donate increasing 1 percent, the tuition will increase 52.8249113 dollars. With instructional expenditure per student increasing 1 unit, the tuition will increase 0.2730896 dollars. With the graduation rate increase by 1 unit, the tuition will increase 24.6504004 dollars.
...
Based on Part 2, I use linear regression on private, Room.Board and perc.alumni, cubic transformation on PhD, log transformation on Expend, and local regression on Grad.Rate.
...
We can see that the GAM model with the Room.Board in linear fashion is statistically significant at '0' level, while two others are at '0.01' level or not statistically significant, so we can conclude that Room.Board has a linear relationship with Outstate tuition.
