...
tidy(model1)
```   
Regress `biden` on `age`, the estimated coefficient of `age`  is `r bi_age$coefficients[2]` with a standard error of `r model1$coefficients[2, 2]`, and the intercept is `r bi_age$coefficients[1]` with a standard error of `r model1$coefficients[1, 2]`. 

1. Is there a relationship between the predictor and the response?  
There is barely a statistically significant relationship between the predictor `age` and the response, Joe Biden feeling thermometer (hereafer `biden`). With a p value of `r model1$coefficients[2, 4]`, the relationship is only marginally significant at $\alpha$ = 0.1 level (meaning that we can reject the null hypothesis - there is no relationship - at a 90% confidence level.) Moreover, the size of effect of `age` is also small, with an estimated coefficient of `r bi_age$coefficients[2]`. Therefore, we conclude that there is a relationship between `age` and `biden`, but it is weak and only marginally significant.
...
The $R^2$ of the model is `r model1$r.squared`, indicating that `age` alone only explains 0.2% of the variation in `biden`. This model does not adequatley explain the variation in the dependent variable, and thus is not a good enough model. 
...
tidy(model2)
```
Regress feeling scores (`biden`) on respondent age (`age`), gender(`female`) and educational level (`educ`), we estimated that the coefficient of `age` is `r bi_mul$coefficients[2]` with a standard error of `r model2$coefficients[2, 2]`; the coefficient of `female` is `r bi_mul$coefficients[3]` with a standard error of `r model2$coefficients[3, 2]`; the coefficient of `educ` is `r bi_mul$coefficients[4]` with a standard error of `r model2$coefficients[4, 2]`.

1. Is there a statistically significant relationship between the predictors and response?  
After we added the predictors of gender and education into the model, `age` becomes completely insignificant with a p value of 0.198, meaning that there is no relationship between respondent age and their feelings toward Biden if hold constant gender and education. Nevertheless, there are statistically significant relationship between respondent' feeling score and their gender (p value = 1.86e-08, significant at $\alpha$ = 0.01 level) and educational level(p value = 7.94e-05, significant at $\alpha$ = 0.01 level), holding age constant.  
...
The $R^2$ of the model is `r model2$r.squared`, indicating that age, gender and education jointly explain 2.72% of the variation in `biden`. Though still not  explain much of the variation, this model has been improved from the age-only model which only explains 0.2%. 
...
The $R^2$ of model 3 is `r model3$r.squared`, meaning that age, gender, educaton and party affiliation jointly explain 28.15% of the variations in `biden`. Even adjusting for the number of parameters added, the Adjusted $R^2$ is still as large as 0.2795. This is a much better model than model 2 (age + gender + education) which only explains about 2% of the variations. 
