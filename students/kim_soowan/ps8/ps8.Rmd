...
The test MSE for this model is `r mse_svm6`, higher than that for the random forest model. The test accuracy rate is `r mh_model_acc6`, meaning it predicted voting accurately `r round(mh_model_acc6*100,2)`% of the time. The PRE is `r pre6`, indicating that it outperforms the useless classifier by a narrow margin. The AUC is `r auc(roc6)[[1]]`, significantly lower than the AUC for the random forest model.
...
guilt_race <- glm(guilt ~ Age, data = titanic, family = binomial)
summary(survive_age)
```
