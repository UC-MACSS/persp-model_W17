...
-   [Clustering states \[3 points\]1](#clustering-states-3-points1)
...
As a whole, MSE decreases as K increases. However, the MSE is minimized at 455.7123299 when K = 45.
...
mh <- read.csv("data/mental_health.csv") %>%
  na.omit()
#mh$vote96 <- as.numeric(mh$vote96)
...
All four sets of predictors perform similarly, but the test error rate is minimized at K = 9 with`vote96 ~ mhealth_sum + age + educ + black + female + married`.
...
![](ps9_files/figure-markdown_github/mh_wknn_mse-1.png)
...
![](ps9_files/figure-markdown_github/unnamed-chunk-1-1.png)

The test error rate is minimized with `vote96 ~ mhealth_sum + age + educ` at K = 9.
...
ggplot() +
  ...
  labs(title = "Weighted KNN for vote96",
       x = "K",
       y = "Test error rate",
       caption = "KNN: vote96 ~ mhealth_sum + age + educ + black + female + married\n
...
The non-weighted `vote96 ~ mhealth_sum + age + educ + black + female + married` performs the best.
...
With K = 2, the states seem cluster by the first principal component. The states are divided roughly in half, with one side on the left and the other on the right. States with higher crime rates cluster in group 1, while states with lower crime rates fall mostly in group 2.
...
![](ps9_files/figure-markdown_github/unnamed-chunk-5-1.png)
...
There is less overlapping among the clusters compared to the results for K = 3 clustering on the raw data. The states are cleanly separated in groups, mostly along the PC1 axis.
...
3.  Hierarchically cluster the states using complete linkage and Euclidean distance, after scaling the variables to have standard deviation 1. What effect does scaling the variables have on the hierarchical clustering obtained? In your opinion, should the variables be scaled before the inter-observation dissimilarities are computed? Provide a justification for your answer.
